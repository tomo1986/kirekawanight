class Shop < ApplicationRecord
  ...
  has_many  :coupons, class_name: 'CouponType::Shop', as: :subject, dependent: :destroy, :autosave => true
  has_many :users
  ...
  scope :global_search, ->(sort=nil,job=nil,tags=nil,budget=nil,mama_tip=nil,tip=nil,charge=nil,karaoke_machine=nil,japanese=nil,english=nil){
    ...
    if budget
      shops = shops.where("shops.budget_usd >= 30 and shops.budget_usd < 41") if budget.to_i == 1
      shops = shops.where("shops.budget_usd >= 41 and shops.budget_usd < 51") if budget.to_i == 2
      shops = shops.where("shops.budget_usd >= 51 and shops.budget_usd < 61") if budget.to_i == 3
      shops = shops.where("shops.budget_usd >= 61 and shops.budget_usd < 71") if budget.to_i == 4
    end
