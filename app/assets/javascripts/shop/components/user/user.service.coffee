...
.factory 'user', (api) ->
  ...
  sm.clear = ->
    api.postPromise('/api/shop/logout',{}).then((res) ->
      sm.login_user = null
  ...
  sm.getUsers = (opt_filter)->
    api.getPromise('/api/shop/api7',opt_filter).then((res) ->
      return res
    )

  sm.newUser = ->
    api.getPromise('/api/shop/api8',{}).then((res) ->
      return res
    )

  sm.createUser = (opt_user)->
    api.postPromise('/api/shop/api9',opt_user).then((res) ->
      return res
    )

  sm.getUser = (opt_user_id) ->
    api.getPromise('/api/shop/api10',{id:opt_user_id}).then((res) ->
      return res
    )


  sm.updateUser = (opt_user) ->
    api.postPromise('/api/shop/api11',opt_user).then((res) ->
      return res
    )

  sm.deleteUser = (opt_user_id) ->
    api.postPromise('/api/shop/api12',opt_user_id).then((res) ->
      return res
    )


  sm.getDate = (opt_id) ->
    api.getPromise('/api/shop/api13',{id: opt_id}).then((res) ->
      return res
    )

  sm.getContacts = (opt_id) ->
    api.getPromise('/api/shop/api14',{id: opt_id}).then((res) ->
      return res
    )
