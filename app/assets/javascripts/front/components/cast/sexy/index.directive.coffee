...
.directive 'castSexyIndexDirective', () ->
  CastSexyIndexController = ($state,castService, customerService, modalService) ->
    ...
    vm.changePageFunk = ->
      $state.go('/casts/sexy',{page:vm.filters.page, sort: vm.filters.sort, order: vm.filters.order})
  ...
  directive =
    restrict: 'A'
    controller: CastSexyIndexController
    controllerAs: 'vm'
