class Api::ShopController < ApiController
  ...
  def logout
    sign_out(current_shop) if shop_signed_in?
  ...
  def api1
    ...
    render_failed(4,t('shop.error.not_find')) and return if email.blank? || password.blank?
    shop = Shop.where(:email => params[:email]).first

    # render_failed(4, t('shop.error.not_find')) and return unless shop && shop.valid_password?(params[:password])
    shop.remember_me = params[:remember_me]
    sign_in shop
  ...
  def api2
    render_failed(4, t('shop.error.no_login')) and return unless shop_signed_in?
    builder = Jbuilder.new do |json|
      json.shop current_shop.to_jbuilder
    end
    render json: builder.target!
  end
  ...
  def api4
    ...
    shop = current_shop
    shop.attributes = {
        group_id: current_shop.group.id,
        name: params[:name],
  ...
  def api7
    ...
    users = User.where(shop_id: current_shop.id, deleted_at: nil)
  ...
  # new user
  def api8
    render_failed(4, t('group.error.no_login')) and return unless shop_signed_in?
    user = User.new
    builder = Jbuilder.new do |json|
      json.code 1
  ...
  def api9
    ...
    user.attributes = {
        shop_id: current_shop.id,
        job_type: current_shop.job_type,
        name: params[:name],
  ...
  def api10
    ...
    render_failed(4, t('shop.error.no_user')) and return unless user
  ...
  def api12
    render_failed(4, t('group.error.no_login')) and return unless shop_signed_in?
    user = User.find_by(id: params[:id]).first
  ...
  def api15
    render_failed(4, t('shop.error.no_login')) and return unless shop_signed_in?
    discounts = Discount.where(type: "DiscountType::Time",subject_type:"Shop", subject_id: current_shop.id).order("id desc")
    builders = Jbuilder.new do |json|
